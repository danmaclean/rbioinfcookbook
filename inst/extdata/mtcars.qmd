...
## Abstract
The mtcars dataset is a well-known dataset in the R community, which contains information about different types of cars and their fuel efficiency. In this analysis, we will explore the relationships between various features of the cars and their fuel efficiency, represented by miles per gallon (mpg). We will use visualization techniques, such as scatter plots and box plots, and statistical methods, such as linear regression, to gain insights into the data.
...
From the scatter plot, we can see a negative relationship between the weight of the car and its fuel efficiency. The heavier the car, the lower its fuel efficiency.

Next, let's fit a linear regression model to predict mpg from wt.
...
From the box plot, we can see that cars with 4 cylinders tend to have higher fuel efficiency compared to cars with 6 or 8 cylinders.
